...
public class Python extends Language
{
    ...
    int time;
    int timeOut;
    public Python(String path,String fileName,String extension,int time)
    {
        this.path=path;
      ...
      try 
        {
            ...
            String compileFileCommand="py "+this.path+this.fileName+"."+this.extension;
            Process processCompile = Runtime.getRuntime().exec(compileFileCommand);
            TimedShell ts=new TimedShell(this,processCompile,this.time);
            ts.start();
    ...
    public void execute()
    {
        try
        {
            ...
            out.write("cd \\ && cd "+this.path+" && py eg1.py <inp.txt> out.txt");
            out.close();
